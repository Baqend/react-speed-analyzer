...
const userMetrics = [
  ...
  },
  {
    ...
    label: "Speed Index",
    tooltip:
      "Represents how quickly the page rendered the user-visible content.",
  },
];
...
const technicalMetrics = [
  {
    ...
    tooltip:
      "Measures the amount of time between creating a connection to the server and downloading the contents.",
  },
  {
    name: "domLoaded",
    label: "DOMContentLoaded",
    tooltip:
      "Represents the time when the initial HTML document has been completely loaded and parsed, without " +
  ...
  },
  {
    ...
    label: "FullyLoaded",
    tooltip:
      "Measures the time from the start of the initial navigation until there was 2 seconds of no network " +
  ...
  },
  {
    ...
    tooltip:
      "Represents the last point in the test when something visually changed on the screen.",
  },
];
...
class ResultMetrics extends Component {
  constructor(props) {
    ...
    this.state = {
      showDetails:
        isSpeedKitComparison && speedKitError ? true : props.showDetails,
    };
  }
  ...
  getHighlightMetrics() {
    ...
    const positiveFactors = Object.entries(testOverview.factors)
      ...
      .sort((curr, prev) => {
        ...
        if (
          curr.metric === this.props.result.mainMetric &&
          prev.metric !== "ttfb"
        ) {
          return -1;
        }
        ...
        if (
          prev.metric === this.props.result.mainMetric ||
          prev.metric === "ttfb"
        ) {
          return 1;
        }
    ...
    return (
      ...
      >
        {this.getHighlightMetrics().map((metric, index) => (
          <div
            key={index}
            className="flex flex-column justify-center box-wrapper"
            style={{
              margin: "10px",
              padding: "40px 24px",
              alignItems: "center",
          ...
          >
            <div className="mb2" style={{ width: "200px", margin: "auto" }}>
              <div
                className="competitor-metric-scale mb1"
                style={{ height: "40px" }}
              />
              <div className="flex flex-row" style={{ height: "40px" }}>
                <div
                  className="speedKit-metric-scale"
                  style={{
                    width:
                      100 -
                      ((competitorData[metric] - speedKitData[metric]) /
            ...
            <div className="text-light-grey mt3">
              {metrics.find((metricEntry) => metricEntry.name === metric).label}
            </div>
            <div className="faster" style={{ flexDirection: "row" }}>
              {calculateAbsolute(competitorData[metric], speedKitData[metric])}{" "}
              Faster (
              <span className="purple factor">
                {this.getFactorValue(
                  competitorData[metric],
                  speedKitData[metric]
                )}
      ...
      <div className="result__details-metrics">
        {metrics
          .map((metric, index) => {
            ...
            const factorValue = this.props.result.useFactor
              ? `${factor}x`
              : `${percent}%`;
            return (
                ...
                <div className="w-100">
                  ...
                  <div className="flex flex-row items-center pt3 pb3">
                    <div className="metric-column text-center">
                      {competitorData[metric.name] ? (
                        <div className="metricValue pl2 pl0-ns">
                          {competitorData[metric.name]} ms
                        </div>
                      ) : (
                    ...
                    <div
                      className="flex flex-column factor-column"
                    ...
                    <div className="metric-column text-center">
                      {speedKitData[metric.name] ? (
                        <div className="metricValue pr2 pr0-ns">
                          {speedKitData[metric.name]} ms
                        </div>
                      ) : (
                        <div className="metricValue">-</div>
                      )}
      ...
      <div className="result__details-metrics">
        ...
        <Collapse
          className={`result-details-collapse ${
            this.state.showDetails ? "" : "fade-out"
          }`}
            ...
            <div className="details-toggle" onClick={this.toggleDetails}>
              {this.state.showDetails ? (
                <span>
                  Hide Metrics{" "}
                  <FontAwesomeIcon
                    className="details-toggle-arrow"
                    icon={faChevronUp}
                  />
                </span>
              ) : (
