...
class ResultVideos extends Component {
  ...
  playVideos = (videoLabel) => {
    ...
    const secondVideo =
      videoLabel === "speedKitVideo" ? "competitorVideo" : "speedKitVideo";
    if (isIE() || isEdge()) {
      if (this[secondVideo]) {
  ...
  handleCompetitorProgress = () => {
    const percent =
      this.competitorVideo.currentTime / this.competitorVideo.duration + 0.05;
    this.setState({
  ...
  handleSpeedKitProgress = () => {
    if (this.speedKitVideo) {
      const percent =
        this.speedKitVideo.currentTime / this.speedKitVideo.duration + 0.05;
      this.setState({
  ...
  componentDidMount() {
    if (this.competitorVideo) {
      this.competitorVideo.addEventListener(
        "playing",
        this.handleCompetitorStarted
      );
      this.competitorVideo.addEventListener(
        "timeupdate",
        this.handleCompetitorProgress
      );
      this.competitorVideo.addEventListener(
        "ended",
        this.handleCompetitorEnded
      );
    }

    if (this.speedKitVideo) {
      this.speedKitVideo.addEventListener(
        "playing",
        this.handleSpeedKitStarted
      );
      this.speedKitVideo.addEventListener(
        "timeupdate",
        this.handleSpeedKitProgress
      );
  ...
  componentWillUnmount() {
    if (this.competitorVideo) {
      this.competitorVideo.removeEventListener(
        "playing",
        this.handleCompetitorStarted
      );
      this.competitorVideo.removeEventListener(
    ...
    if (this.speedKitVideo) {
      this.speedKitVideo.removeEventListener(
        "playing",
        this.handleSpeedKitStarted
      );
      this.speedKitVideo.removeEventListener(
  ...
  render() {
    ...
    return (
      <div className="flex justify-center">
        <div
          className={`w-50 competitor-video ${
            this.props.testOverview.mobile ? "mobile" : ""
          }`}
        >
            ...
            <div className="video__wrapper-inner">
              <div className="relative">
                <video
                  ...
                  ref={(video) => {
                    this.competitorVideo = video;
                  }}
                  onClick={() => this.playVideos("competitorVideo")}
                  onPlay={() => this.playVideos("competitorVideo")}
                  src={
                    competitorVideoPath &&
                    `https://${process.env.REACT_APP_BAQEND}/v1${competitorVideoPath}`
                  }
                />
                ...
                {this.competitorVideo && this.competitorVideo.paused && (
                  <div className={"video__wrapper-play"}>
                    <div
                      className="video__wrapper-play-inner dark-blue"
                      onClick={() => this.playVideos("competitorVideo")}
                    >
                  ...
                  <div className="video__wrapper-progress-inner">
                    <div
                      ...
                      style={{
                        ...
                        transition: this.state.isRunningCompetitor
                          ? "all 0.5s linear"
                          : "all 0.01ms linear",
                      }}
                    ></div>
            ...
            <div className="video__wrapper-inner">
              <div className="relative">
                <video
                  ...
                  className="embedVideo"
                  ref={(video) => {
                    this.speedKitVideo = video;
                  }}
                  onClick={() => this.playVideos("speedKitVideo")}
                  onPlay={() => this.playVideos("speedKitVideo")}
                  src={
                    speedKitVideoPath &&
                    `https://${process.env.REACT_APP_BAQEND}/v1${speedKitVideoPath}`
                  }
                />
                  ...
                  <div className="video__wrapper-progress-inner">
                    <div
                      ...
                      style={{
                        ...
                        transition: this.state.isRunningSpeedKit
                          ? "all 0.5s linear"
                          : "all 0.01ms linear",
                      }}
                    ></div>
